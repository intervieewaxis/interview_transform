...
import JobDescriptionField from "../../_components/JobDescriptionField";
...
export const ResumeDialog = () => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { title: "", slug: "", jd: "", file: "", data: "" },
  });
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (isCreate) {
      try {
        ...
        formData.append("job_description", values.jd || "");

        setIsCreating(true)

        // First API call: Upload file
        const uploadResponse = await fetch("https://api.interviewaxis.com/transform/upload_file", {
          method: "POST",
          body: formData,
        });
        ...
        console.log("Initial API response:", uploadResult);

        const creationOutput = await createResume({
          slug: values.slug,
          title: values.title,
          data: uploadResult,
          visibility: "private",
        ...
        console.log("Creation Output : ", creationOutput);
  ...
  const onReset = () => {
    if (isCreate) form.reset({ title: "", slug: "", jd: "" });
    if (isUpdate)
